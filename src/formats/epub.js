...
async function epub (file, extract, {sanitize = true}) {
  ...
  */
  ...
  urls[path.join(file, 'index.json')] = await extract(bookStream, {contentType: "application/json"} );
  ...
  if (sanitize) {
    for (const resource of book.resources) {
      if (resource.encodingFormat.includes('html') || resource.encodingFormat.includes('svg')) {
        const file = await fs.promises.readFile(path.join(tempDirectory, resource.url), 'utf8')
