...
DOMElement.prototype = {
  getAttribute: function DOMElementGetAttribute(name) {
  ...
  getAttributeNS: function DOMElementGetAttributeNS(NS, name) {
    // Fast path
    if (name in this.attributes) {
      return this.attributes[name];
    }
  ...
  setAttribute: function DOMElementSetAttribute(name, value) {
    value = value || "";
    value = xmlEncode(value);
    this.attributes[name] = value;
  },

  setAttributeNS: function DOMElementSetAttributeNS(NS, name, value) {
    this.setAttribute(name, value);
  },

  appendChild: function DOMElementAppendChild(element) {
  ...
  hasChildNodes: function DOMElementHasChildNodes() {
    return this.childNodes.length !== 0;
  },

  cloneNode: function DOMElementCloneNode() {
  ...
  toString: function DOMElementToString() {
    var buf = [];
  ...
  getSerializer: function DOMElementGetSerializer() {
...
DOMElementSerializer.prototype = {
   ...
   */
  getNext: function DOMElementSerializerGetNext() {
    var node = this._node;
    node.nodeName = node.nodeName.replace("svg:", "");
    switch (this._state) {
      ...
      case 3: // Serialize any attributes and end opening tag.
        if (this._loopIndex < this._attributeKeys.length) {
          ...
          if (
            (name === "x" || name === "y" || name === "offset") &&
            !xmlEncode(node.attributes[name])
          ) {
            return " " + name + '="' + 0 + '"';
          } else {
      ...
      case 4: // Serialize textContent for tspan/style elements.
        if (node.nodeName === "tspan" || node.nodeName === "style") {
          this._state = 6;
...
var exportedSymbols = Object.keys(exports);

exports.setStubs = function(namespace) {
  exportedSymbols.forEach(function(key) {
